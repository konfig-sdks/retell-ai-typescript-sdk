...
import { readableStreamToString, RetellAiError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new RetellAiError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new RetellAiError(e, e.response?.data)
                }
                if (innerError instanceof RetellAiError) {
                    // Got "RetellAiError" from the above try block
                    throw innerError;
                }
